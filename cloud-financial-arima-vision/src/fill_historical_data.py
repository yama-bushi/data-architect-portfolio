...
# Example usage:
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'your creds here'
...
def fill_historical_data(symbol, resolution):
    ...
    try:
        ...
        #else:
        query = "SELECT DATETIME_ADD(min(DATETIME_ADD(cast(datetime as datetime), INTERVAL -1 MINUTE)),INTERVAL -30 DAY) dt,min(DATETIME_ADD(cast(datetime as datetime), INTERVAL -1 MINUTE)) end_date FROM `"f"{PROJECT_ID}."f"{symbol_integ}."f"{resolution}`"
        #print(query)
